...
def _read_tsv_input_file(file_full_name, subtask):
    predictions = {}
...
def evaluate(pred_labels, gold_labels, subtask):
    ...
    if type(pred_labels) is not str:
        ...
        pred_labels = _read_tsv_input_file(pred_labels, subtask)
        gold_labels = _read_gold_labels_file(gold_labels, subtask)
...
def evaluate_1C(pred_labels, gold_labels):
    ...
    if type(pred_labels) is not str:
        ...
        pred_labels = _read_tsv_input_file(pred_labels, "1C")
        gold_labels = _read_gold_labels_file(gold_labels, "1C")
        pred_values, gold_values = _extract_matching_lists_1C(pred_labels, gold_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subtask", "-s", required=True, choices=['1A', '1B', '1C'], help="The subtask for which we are checking format: 2A, 2B", type=str)
    ...
    if validate_files(pred_files_path, subtask):
        techniques = []
        ...
        for pred_file_path in pred_files_path:
            ...
            if check_format(pred_file_path, subtask):
                ...
                pred_labels = _read_tsv_input_file(pred_file_path, subtask)
